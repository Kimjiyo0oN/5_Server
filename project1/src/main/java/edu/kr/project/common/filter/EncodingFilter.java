package edu.kr.project.common.filter;
 ...
 * 	알맞은 Servlet으로 전달되기 전 특정 코드를 수행하는 클래스가 필터
 * 
 *  클라이언트가 요청을 하고 HttpServletRequest  객체가 생성 알맞은 Servlet으로 전달하는 게 기본 흐름
 *  						HttpServletResponse  
 *  								|
 *  							    \/
 *  클라이언트가 요청을 하고 HttpServletRequest  객체가 생성 -> [필터] -> 알맞은 Servlet으로 전달하는 게 기본 흐름
 *  						HttpServletResponse  
 * 
...
// @WebFilter 어노테이션 : filtering을 지정할 요청 주소를 작성하는 어노테이션 + 필터로 등록
...
public class EncodingFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// pass the request along the filter chain 꼭 있어야하는 구문
